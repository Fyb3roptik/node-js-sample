...
abstract class User extends Object {
	...
	/**
	 * If a subclass has this user type. Then they are just a regular Customer.
	 */
	const TYPE_CUSTOMER = 'customer';
	/**
	 * If a subclass has this user type. Then they are a Sales_Rep.
	 */
	const TYPE_SALES = 'sales';

	/**
	 * If a subclass has this user type. It's an Admin.
	 */
	 ...
	 */
	public function setPassword($new_password, $old_password = null, $admin_override = false) {
		$password_changed = false;
		$new_password = trim($new_password);
		$current_password = $this->_data['password'];

		//setting a new password from an old password
		if(false == is_null($current_password) && false==empty($current_password) && $current_password != passwordify($old_password, $this->salt) && false == $admin_override) {
			$error = true;
		}
...
/**
 * Recovers a password for a given customer.
 *
 * TODO: make the email to the customer better.
 */
function recover_customer_password(User $U) {
	recover_user_password($U);
}
